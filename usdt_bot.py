...
MESSAGES = {
    ...
    "payment_info": {
        "en": (
            ...
            f"• *AFN Equivalent:* {{total_afn:.2f}} AFN (Rate: {USD_TO_AFN})\n"
            "----------------------------\n\n"
        ...
        "ps": (
            "💵 *د تادیې لارښوونې*\n"
            "----------------------------\n"
            f"• *ټول مبلغ:* {{total:.2f}} USD\n"
            f"• *په افغانی کې:* {{total_afn:.2f}} AFN (نرخ: {USD_TO_AFN})\n"
            "----------------------------\n\n"
        ...
        "fa": (
            "💵 *راهنمای پرداخت*\n"
            "----------------------------\n"
            f"• *مبلغ کل:* {{total:.2f}} USD\n"
            f"• *معادل افغانی:* {{total_afn:.2f}} AFN (نرخ: {USD_TO_AFN})\n"
            "----------------------------\n\n"
...
async def wallet_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = MESSAGES["payment_info"][lang].format(total=total, total_afn=total_afn, hesabpay=HESABPAY_ID)
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "done":
        ...
        await query.edit_message_caption(caption="✅ Order marked as DONE by admin.")
    elif action == "cancel":
...
async def set_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text(f"Current AFN rate: {USD_TO_AFN}")
        return
    if args[0].lower() == "live":
        # keep simple: no live fetch, admin can set numeric rate
        await update.message.reply_text("Live fetch not configured. Use /rate <number>")
        return
    try:
        USD_TO_AFN = float(args[0])
        await update.message.reply_text(f"Rate updated to {USD_TO_AFN} AFN per 1 USD")
    except:
        await update.message.reply_text("Invalid format. Use: /rate 67")
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data or "total" not in data:
        ...
        return
    await update.message.reply_text(f"Latest order: {data}")
