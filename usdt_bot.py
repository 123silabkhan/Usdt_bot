...
# ---------------------------------------------------------
# 🔹 YOUR TELEGRAM TOKEN (PythonAnywhere FREE requires direct)
# ---------------------------------------------------------
TELEGRAM_TOKEN = "8512558373:AAEw1dEOdoG3NtO1VGsCSFOyldAlRStDdvQ"

# 🔹 Admin & HesabPay Info
ADMIN_ID = 6491173992
HESABPAY_ID = "0729376719"
...
async def wallet_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_data_store[user_id]["wallet"] = update.message.text.strip()
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "done":
        await context.bot.send_message(
            chat_id=user_id,
            text=MESSAGES["approved_user"][lang]
        )
        await query.edit_message_caption("✅ Order marked as DONE by admin.")

    elif action == "cancel":
        await context.bot.send_message(
            chat_id=user_id,
            text=MESSAGES["rejected_user"][lang]
        )
        await query.edit_message_caption("❌ Order REJECTED by admin.")
